...
### Added
- VCSスキーム切り替え機能の実装
- E2Eテスト用の自動プロジェクト選択機能とスクロール方法の追加
- MarkdownプレビューでMermaid図のレンダリング対応
- Remote UI存在確認機能の実装
- spec/bugワークフロー用マージスクリプトの統合
- アイドル時間トラッキングとプロジェクトレベルレポート機能
- プロジェクトエディターのダークモード対応
- ツールパス解決と設定UI機能（well-known tool paths）
- リモートE2E実行機能の追加

### Fixed
- Electron版でmarkdownFilesタブが表示されない問題を修正


## [0.68.2] - 2026-02-05

### Fixed
- ターミナル起動時のzsh TTYエラーとセッション保存メッセージを修正
- VSCodeシェル統合のANSIエスケープシーケンスがコマンドパスに混入する問題を修正


## [0.68.1] - 2026-02-04

### Fixed
- claudeパス解決でzshセッション復元メッセージが混入する問題を修正
...
### Added
- E2E Pipelineをデフォルト有効化、--skip-e2eオプション追加
- WebSocketコマンドAPIの統一化
- Claude実行パスの自動解決機能を追加
- E2Eエージェントとテンプレートのインストール対応
...
### Added
- スケジュールタスクのスケジューラ起動機能を実装

### Fixed
- UI統合漏れを修正しrebaseボタンを表示
- chokidar v4+でグロブパターン非対応のため監視方式を修正

### Changed
- chokidar v4+推奨の関数ベースignoredに統一
