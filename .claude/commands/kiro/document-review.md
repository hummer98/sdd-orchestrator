...
Collect all `.md` and `.json` files from `.kiro/specs/$0/` directory:
...
1. Search for `document-review-*.md` files in `.kiro/specs/$0/`
2. Get maximum existing review number
3. Determine next sequential number (start with `1` if none exist)

Report file: `.kiro/specs/$0/document-review-{n}.md`
...
1. **Report Location**: `.kiro/specs/$0/document-review-{n}.md`
...
- Run `/kiro:document-review-reply $0` to generate responses to review issues
- Address critical issues based on the response recommendations
- Update relevant specification documents
- Re-run `/kiro:document-review $0` to verify fixes

**If Warnings Only**:

- Run `/kiro:document-review-reply $0` to review and respond to warnings
- Or document accepted risks and proceed
- Run `/kiro:spec-impl $0` when ready

**If Clean Review**:

- Specification is ready for implementation
- Run `/kiro:spec-impl $0` to start development
