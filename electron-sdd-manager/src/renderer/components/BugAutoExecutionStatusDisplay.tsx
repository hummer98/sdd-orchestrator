/**
 * BugAutoExecutionStatusDisplay Component
 * Displays bug auto execution progress and status
 * bugs-workflow-auto-execution Task 3
...
// ============================================================
// Task 3.1: BugAutoExecutionStatusDisplay Props
// Requirements: 3.1
// ============================================================

export interface BugAutoExecutionStatusDisplayProps {
  /** 自動実行状態 */
...
export function BugAutoExecutionStatusDisplay({
  status,
  currentPhase,
  lastFailedPhase,
  retryCount,
  onRetry,
  onStop,
}: BugAutoExecutionStatusDisplayProps) {
  // Don't render when idle (Requirements: 3.5)
  if (status === 'idle') {
    return null;
  }
  ...
  return (
    <div
      className={clsx(
        ...
        'transition-colors',
        status === 'running' && 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20',
        status === 'paused' && 'border-yellow-200 bg-yellow-50 dark:border-yellow-800 dark:bg-yellow-900/20',
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {/* Status Text (Requirements: 3.1, 3.2, 3.3, 3.4) */}
          <span
            className={clsx(
              'font-medium',
          ...
          {/* Current Phase (Requirements: 3.1) */}
          {(status === 'running' || status === 'paused') && currentPhase && (
            <span className="text-sm text-gray-600 dark:text-gray-400">
              - {BUG_WORKFLOW_PHASE_LABELS[currentPhase]}
            </span>
          )}

          {/* Failed Phase (Requirements: 3.4) */}
          {status === 'error' && lastFailedPhase && (
            <span className="text-sm text-red-600 dark:text-red-400">
              - {BUG_WORKFLOW_PHASE_LABELS[lastFailedPhase]}で失敗
            </span>
          )}

          {/* Retry Count (Requirements: 5.3) */}
          {retryCount > 0 && (
        ...
        {/* Action Buttons (Requirements: 5.1, 6.4) */}
        <div className="flex items-center gap-2">
          ...
          {/* Retry Button - shown when error (Requirements: 5.1) */}
          {status === 'error' && (
            <button
              onClick={onRetry}
              className={clsx(
