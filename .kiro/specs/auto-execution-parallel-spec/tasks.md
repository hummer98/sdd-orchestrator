...
- [x] 1. ExecutionContext型定義と基盤実装
- [x] 1.1 (P) ExecutionContext型をTypeScriptで定義する
...
- [x] 1.2 (P) ExecutionContext生成ファクトリ関数を実装する
...
- [x] 2. AutoExecutionServiceの内部状態管理を再構築する
- [x] 2.1 クラス変数をMap形式に置き換える
...
- [x] 3. start()メソッドを並行実行対応に改修する
- [x] 3.1 ExecutionContext作成ロジックを統合する
...
- [x] 3.2 executePhase呼び出し後のagentIdマッピング登録を実装する
...
- [x] 4. Agent完了イベントハンドリングを改修する
- [x] 4.1 handleDirectStatusChange()でagentToSpecMapからSpecIdを解決する
...
- [x] 4.2 handleAgentCompleted()をExecutionContext単位で動作させる
...
- [x] 10. UI状態表示の検証と調整
- [x] 10.1 WorkflowViewが選択中Specのruntime状態のみ表示することを検証する
  - specStore.getAutoExecutionRuntime(selectedSpec.name)の呼び出しを確認
  - Spec切り替え時のUI更新動作を確認
  - 既存実装が要件を満たしていれば変更不要
  - _Requirements: 5.1, 5.2, 5.3, 5.5_

- [x] 10.2 (P) specStore.getAutoExecutionRuntime()が正しく動作することを検証する
...
- [x] 11. 単体テストを実装する
- [x] 11.1 (P) ExecutionContext作成のテストを実装する
  - specDetailスナップショットが正しく保存される
  - specPathがコンテキストに含まれる
  - 初期状態が正しく設定される
  - _Requirements: 1.5, 1.6_

- [x] 11.2 (P) agentToSpecMapマッピングのテストを実装する
  - AgentId登録が正しく動作する
  - AgentId削除が正しく動作する
...
- [x] 11.3 (P) 並行実行上限チェックのテストを実装する
  - 5件までの実行が許可される
  - 6件目の実行がfalseを返す
  - 同一specIdの重複実行が防止される
  - _Requirements: 3.4, 3.5_

- [x] 11.4 (P) handleDirectStatusChange正確性のテストを実装する
  - 正しいExecutionContextが更新される
  - 他のContextに影響しない
  - バッファリングされたイベントが後で処理される
  - _Requirements: 1.3, 2.3, 3.2_

- [x] 11.5 (P) Context独立性のテストを実装する
  - Spec Aのエラーが Spec Bに影響しない
  - Spec Aの完了が Spec Bに影響しない
  - 各Contextが独立したタイムアウトを持つ
  - _Requirements: 3.1, 3.2, 3.3, 3.6_

- [x] 11.6 (P) クリーンアップ動作のテストを実装する
  - 完了後2秒遅延でContextが削除される
  - 手動停止で即座にクリーンアップされる
  - dispose()で全Contextがクリアされる
  - _Requirements: 6.1, 6.3, 6.4_

- [x] 11.7 (P) 後方互換性のテストを実装する
  - 単一Spec実行時の動作が既存と同一
  - 既存APIシグネチャが維持される
  - 通知パターンが保持される
  - _Requirements: 7.1, 7.3, 7.4, 7.5_

- [x] 12. 統合テストとE2Eテストを実装する
- [x] 12.1 並行実行フローの統合テストを実装する
  - 2つのSpecを同時に自動実行開始
  - 両方のAgent完了イベントが正しくルーティングされる
  - 両方のSpecが正常に完了する
  - _Requirements: 3.1, 3.2_

- [x] 12.2 (P) UI状態表示の統合テストを実装する
