...
| 判断ポイント | Type |
|-------------|------|
| ユーザーにとって新しくできることが増える | `feat` |
| 壊れていた動作が期待通りになる | `fix` |
| 動作は変わらないがコード構造が改善される | `refactor` |

**注意: Spec名から Type を推測しない**

Spec名に含まれるキーワード（fix, unification, refine等）は変更の動機を表すが、
コミットの Type は**実際の変更内容**で判断する。

| Spec名のキーワード | 実際の変更内容 | 正しい Type |
|-------------------|---------------|-------------|
| `-fix` | 新APIを追加して問題を解決 | `feat` |
| `-fix` | 既存ロジックの条件分岐を修正 | `fix` |
| `-unification` | UI/ロジックの共通化 | `refactor` |
| `-refine` | 既存機能の改善 | `feat` or `refactor` |
