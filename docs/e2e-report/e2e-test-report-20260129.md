...
#### 2.2 bugs-pane-integration.e2e.spec.ts (11 failing)

**症状**: バグ選択状態が正しく反映されない

```
Bug未選択時にメインペインが空状態を表示
Expected: true, Received: false
```

**分析**: bugStore の selectedBug 状態が期待通りに動作していない。

**要調査**: バグ選択ロジックとStore状態管理

---

#### 2.3 auto-execution-permissions.e2e.spec.ts (5 failing)

**症状**: パーミッション設定が正しく反映されない

```
should verify all permissions are OFF
Expected: false, Received: true
```

**分析**: パーミッション設定の読み込み、または設定後のUI反映に問題。

**要調査**: skipPermissions 設定の保存・読み込みロジック

---

#### 2.4 auto-execution-workflow.e2e.spec.ts (4 failing)
...
#### 2.5 auto-execution-document-review.e2e.spec.ts (3 failing)
...
**分析**: ドキュメントレビューのpauseフラグが機能していない。

**要調査**: documentReviewPermission の "pause" モードの実装
...
2. **bugs-pane-integration.e2e.spec.ts** - 11件の失敗。バグペイン機能の確認
3. **auto-execution-permissions.e2e.spec.ts** - パーミッション設定の問題
...
2. bugStore の selectedBug 状態管理の確認
3. auto-execution パーミッション設定の確認
4. worktree 関連の残りのテスト失敗を調査
