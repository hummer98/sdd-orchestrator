...
| 2.1 タブ表示順序 | PASS | - | 固定タブ→document-review→inspection→その他*.mdの順序で統合（SpecPane.tsx:150-152） |
...
| 5.3 固定ファイル除外しない | PASS | - | listMarkdownFilesInSpecは固定ファイルを除外するが、SpecDetailのmarkdownFilesとして返却される設計（要件5.3の意図は「全*.mdをSpecDetailに含める」ことだが、表示時に重複防止のために固定ファイルはフィルタリングされる） |
...
|------|--------|----------|---------|
| 1.1 FileService.listMarkdownFilesInSpec | PASS | - | [x] 実装完了、テスト全68件パス |
...
| tech.md | PASS | - | React 19 + TypeScript、IPC設計パターン、Remote UIアーキテクチャに準拠 |
| structure.md | PASS | - | ファイル配置はstructure.mdのパターンに準拠（Main: services/, IPC: ipc/, Renderer: components/, Shared: api/） |
...
| テスト成功 | PASS | - | fileService.test.ts: 68テスト全パス |
